...
function loadProfile() {
  ...
  return {
    name: "Sanket Thakare",
    email: "thakaresanket2006@gmail.com",
    bio: "Passionate computer science student aiming for DSA mastery and AI specialization.",
    targetHours: "4",
    avatarUrl: null,
    education: {
      degree: "B.Tech",
    ...
    courses: [
      { id: "c1", name: "Data Structures & Algorithms", status: "completed", grade: "A", credits: 4 },
      { id: "c2", name: "Machine Learning", status: "in-progress", grade: "", credits: 4 },
      { id: "c3", name: "Operating Systems", status: "in-progress", grade: "", credits: 3 },
      { id: "c4", name: "Discrete Mathematics", status: "completed", grade: "A+", credits: 3 },
      { id: "c5", name: "Database Management", status: "completed", grade: "B+", credits: 3 },
      { id: "c6", name: "Computer Networks", status: "pending", grade: "", credits: 3 },
    ],
  };
}
...
function ProfilePage() {
  ...
  const [newCourse, setNewCourse] = useState({ name: "", status: "in-progress", grade: "", credits: 3 });
  ...
  const setCourseField = (id, key, value) =>
    setProfile((p) => ({ ...p, courses: p.courses.map((c) => (c.id === id ? { ...c, [key]: value } : c)) }));
  ...
  const addCourse = () => {
    ...
    setProfile((p) => ({
      ...p,
      courses: [...p.courses, { ...newCourse, id: `c_${Date.now()}` }],
    }));
    setNewCourse({ name: "", status: "in-progress", grade: "", credits: 3 });
                  ...
                  <div className="border-t border-border">
                    ...
                    <div className="divide-y divide-border/60">
                      {profile.courses.map((course) => {
                        ...
                        return (
                          <div key={course.id} className="flex flex-wrap items-center gap-3 px-6 py-3 hover:bg-muted/20 transition-colors">
                            <Icon className={cn("h-4 w-4 shrink-0",
                              course.status === "completed" ? "text-emerald-500" :
                              course.status === "in-progress" ? "text-amber-500" : "text-muted-foreground"
                            )} />
                            <input
                              type="text"
                              value={course.name}
                              onChange={(e) => setCourseField(course.id, "name", e.target.value)}
                              className="flex-1 min-w-[140px] bg-transparent text-sm font-medium outline-none border-b border-transparent focus:border-primary transition-all py-0.5"
                              placeholder="Course name"
                            />
                            <select
                              value={course.status}
                              onChange={(e) => setCourseField(course.id, "status", e.target.value)}
                              className={cn("text-xs font-semibold rounded-full px-3 py-1 border outline-none cursor-pointer", STATUS_COLORS[course.status])}
                            >
                            ...
                            {course.status === "completed" && (
                              <input
                                type="text"
                                value={course.grade}
                                onChange={(e) => setCourseField(course.id, "grade", e.target.value)}
                                className="w-14 text-center text-xs font-bold rounded-lg border border-input bg-card py-1 px-2 outline-none focus:border-primary transition-all"
                                placeholder="Grade"
                              />
                            )}
                            <input
                              ...
                              onChange={(e) => setCourseField(course.id, "credits", Number(e.target.value))}
                              title="Credits"
                              className="w-12 text-center text-xs font-semibold rounded-lg border border-input bg-card py-1 px-1 outline-none focus:border-primary transition-all"
                            />
                      ...
                      <AnimatePresence>
                        {showAddCourse ? (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -8 }}
                            className="flex flex-wrap items-center gap-3 p-3 rounded-xl bg-muted/40 border border-border"
                          >
                            <input
                              type="text"
                              value={newCourse.name}
                              onChange={(e) => setNewCourse((c) => ({ ...c, name: e.target.value }))}
                              className="flex-1 min-w-[160px] bg-card border border-input rounded-lg px-3 py-1.5 text-sm font-medium outline-none focus:border-primary transition-all"
                              placeholder="Course name"
                              autoFocus
                            />
                            <select
                              value={newCourse.status}
                              onChange={(e) => setNewCourse((c) => ({ ...c, status: e.target.value }))}
                              className={cn("text-xs font-semibold rounded-full px-3 py-1 border outline-none cursor-pointer", STATUS_COLORS[newCourse.status])}
                            >
                              <option value="in-progress">In Progress</option>
                              <option value="completed">Completed</option>
                              <option value="pending">Pending</option>
                            ...
                            <input
                              ...
                              placeholder="Grade"
                              className="w-16 text-center text-xs rounded-lg border border-input bg-card py-1.5 px-2 outline-none focus:border-primary"
                            />
                            <input
                              type="number"
                              min="1" max="6"
                              value={newCourse.credits}
                              onChange={(e) => setNewCourse((c) => ({ ...c, credits: Number(e.target.value) }))}
                              className="w-14 text-center text-xs rounded-lg border border-input bg-card py-1.5 px-2 outline-none focus:border-primary"
                            />
                            <span className="text-[10px] text-muted-foreground -ml-1">cr</span>
                            <button onClick={addCourse} className="btn-gradient rounded-lg px-3 py-1.5 text-xs font-bold">Add</button>
                            <button onClick={() => setShowAddCourse(false)} className="text-xs text-muted-foreground hover:text-foreground">Cancel</button>
                          </motion.div>
                        ) : (
                          <button
                          ...
                          >
                            <Plus className="h-4 w-4" /> Add Course / Subject
                          </button>
                        )}
                      </AnimatePresence>
                    </div>
