...
function _nullishCoalesce(lhs, rhsFn) {
    ...
    const Route = _reactrouter.createFileRoute("/app/planner")({
      head: () => ({ meta: [{ title: "Study Planner \u2014 VediQ" }] }),
    ...
    function PlannerPage() {
      ...
      const [subjectList, setSubjectList] = _react.useState(() => {
        ...
        // New user: start with empty list, not mock data
        return [];
      });
      _react.useEffect(() => {
        if (typeof window !== "undefined") {
      ...
      _react.useEffect(() => {
        const onStorage = (e) => {
          if (e.key === "nw_subjects") {
            try {
              const updated = JSON.parse(localStorage.getItem("nw_subjects") || "null");
          ...
          _jsxdevruntime.jsxDEV(_framermotion.motion.div, { variants: _kit.fadeUp, className: "card-surface p-6 xl:col-span-2", children: [
            ...
            subjectList.length === 0
              ? _jsxdevruntime.jsxDEV("div", { className: "mt-8 flex flex-col items-center justify-center text-center gap-3 py-8", children: [
                  ...
                  _jsxdevruntime.jsxDEV("p", { className: "text-sm font-semibold text-muted-foreground" }, void 0, false),
                  _jsxdevruntime.jsxDEV("p", { className: "text-xs text-muted-foreground/70 max-w-xs" }, void 0, false),
                  _jsxdevruntime.jsxDEV("p", { className: "font-semibold text-sm text-foreground", children: "No subjects yet" }, void 0, false),
                  _jsxdevruntime.jsxDEV("p", { className: "text-xs text-muted-foreground max-w-xs", children: "Add subjects via your profile or click \"Add subject\" above to get started." }, void 0, false),
                  _jsxdevruntime.jsxDEV(_reactrouter.Link, { to: "/app/profile", className: "mt-2 btn-gradient rounded-xl px-4 py-2 text-xs font-bold", children: "Go to Profile →" }, void 0, false)
                ] }, void 0, true)
              : _jsxdevruntime.jsxDEV("div", {
