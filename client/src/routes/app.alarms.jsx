  ...
  return (
    ...
    >
      <div className="text-center w-full max-w-md space-y-6 my-4">
        ...
        {/* Dismiss Alarm Button (Direct click allowed ONLY for low/medium risk; high risk requires QR scan) */}
        {!mlLoading && !solved && (
          <div className="flex flex-col gap-3">
            {risk !== "high" && (
              <button
                onClick={handleDismissClick}
                className="btn-gradient w-full rounded-xl py-3.5 text-sm font-bold text-white shadow-lift transition-all"
              >
                Dismiss Alarm
              </button>
            )}
            <p className="text-xs text-muted-foreground">
              {risk === "high" 
                ? "🔒 High snooze risk detected: You must scan the QR code above to stop this alarm!"
                : risk === "medium" 
                ? "🟡 Moderate risk: stay committed to your study schedule today!"
                : "✅ Low risk: great wake habit, click dismiss to complete!"
              }
            </p>
          </div>
        )}
